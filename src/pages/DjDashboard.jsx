    ...
    return (
      <div className="min-h-screen px-6 py-10">
        <div className="mx-auto w-full max-w-6xl space-y-6">
          ...
          <section className="card-vercel p-6">
            <div className="flex items-center justify-between gap-3 mb-4">
              <h3 className="text-xl font-semibold">Comisiones por DJ</h3>
              {adminLoading && (
                <span className="text-sm text-white/40">Actualizando...</span>
              )}
            </div>
            {djNotice && (
              <p className="mb-3 text-sm text-emerald-300">{djNotice}</p>
            )}
            {djError && <p className="mb-3 text-sm text-red-300">{djError}</p>}
            <div className="space-y-3">
              {adminDjs.length === 0 ? (
                ...
                adminDjs.map((dj) => (
                  <div
                    key={dj.id}
                    className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-background/60 p-4 ring-1 ring-border"
                  >
                    <div>
                      ...
                      <p className="text-xs text-white/50">
                        Stripe:{" "}
                        {dj.stripeAccountId ? "Conectado" : "Sin conectar"}
                      </p>
                      <p className="text-xs text-white/50">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="input-vercel h-9 w-24 text-white"
                        onBlur={(e) =>
  ...
  if (user.role !== "DJ") {
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="card-vercel rounded-xl p-6 text-center space-y-4">
  ...
  return (
    ...
    >
      {activeSection === "home" && (
        <section className="card-vercel p-6">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <button
              className="btn-secondary h-12 justify-start px-4"
      ...
      {activeSection === "payments" && (
        <section className="grid gap-4 md:grid-cols-2">
          <div className="card-vercel p-5">
            ...
            <button
              className="btn-secondary mt-3 w-full h-10 disabled:opacity-50"
              type="button"
              onClick={connectStripe}
              disabled={stripeLoading}
            >
              {stripeStatus.connected ? "Gestionar Stripe" : "Conectar Stripe"}
            </button>
          ...
          <div className="space-y-4">
            {!event ? (
              <form
                ...
                className="card-vercel space-y-4 p-6"
              >
      ...
      {activeSection === "settings" && (
        <section className="card-vercel p-6">
          <p className="text-sm text-white/60">Ajustes</p>
          <p className="mt-2 text-sm text-white/50">
            Próximamente: límites, cooldowns y configuración del evento.
          </p>
        </section>
      )}
    </DJLayout>
